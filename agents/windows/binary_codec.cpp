...
std::vector<unsigned char> Sha256Digest(const std::vector<unsigned char>& data) {
  ...
  AppendBigEndian64(&message, bit_length);

  for (size_t chunk = 0; chunk < message.size(); chunk += 64) {
    uint32_t w[64] = {};
    for (int index = 0; index < 16; index += 1) {
      w[index] = ReadBigEndian32(&message[chunk + static_cast<size_t>(index * 4)]);
    }
  ...
  std::vector<unsigned char> output;
  output.reserve(32);
  for (const uint32_t value : h) {
    AppendBigEndian32Bytes(&output, value);
  }
  return output;
}
